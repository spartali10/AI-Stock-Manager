...
const script = fs.readFileSync(path.join(root, 'js/logout.js'), 'utf8');
let pages = 0;
...
events.click({ ...event, target: { closest: () => null } }); assert.equal(prompts, 0);
events.click(event); assert.equal(removed.length, 0); assert.equal(redirects.length, 0);
confirmation = true; events.click(event);
...
events.click(event); assert.equal(redirects.length, 2);
let stopped = false;
