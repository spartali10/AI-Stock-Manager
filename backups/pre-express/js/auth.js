(() => {
    ...
    const pagePermissions = { 'stok.html': 'stock', 'stokyonetimi.html': 'stock', 'magazalar.html': 'stores', 'satis-analizi.html': 'sales', 'raporlar.html': 'reports', 'transferler.html': 'transfers', 'ai-onerileri.html': 'ai', 'bildirimler.html': 'notifications', 'kullanicilar.html': 'admin', 'ayarlar.html': 'admin', 'entegrasyon.html': 'admin', 'anasayfa.html': 'dashboard', 'index.html': 'dashboard' };
    const roles = { admin: 'Admin', manager: 'Müdür', editor: 'Operasyon', viewer: 'Görüntüleyici' };
    const defaults = { admin: [], manager: ['stock', 'stores'], editor: ['stock'], viewer: ['stock'] };
    function users() {
    ...
    function migrate() {
        ...
        all.forEach(u => {
            ...
            if (!roles[u.role]) { u.role = u.role === 'Sistem Yöneticisi' || u.role === 'Yönetici' ? 'admin' : 'viewer'; changed = true; }
    ...
    function landing() { return Object.keys(pagePermissions).find(page => can(pagePermissions[page])) || 'erisim-yok.html'; }
    ...
    function guard() {
        ...
        if (!u || pagePermissions[page] && !can(pagePermissions[page])) {
            ...
            location.replace(u ? landing() : 'login.html'); return;
        }
        localStorage.setItem('aiStockUser', JSON.stringify(clean(u)));
    }
    function applyUI() {
        ...
        document.querySelectorAll('a[href]').forEach(a => { const page = a.getAttribute('href').split('#')[0].split('/').pop(); if (pagePermissions[page] && !can(pagePermissions[page])) a.hidden = true; });
    ...
    try { migrate(); guard(); } catch (error) { if (!location.pathname.endsWith('login.html')) { document.documentElement.style.visibility = 'hidden'; location.replace('login.html'); } }
