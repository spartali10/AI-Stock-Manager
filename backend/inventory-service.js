// Existing inventory and transfer rules, with injected transactional persistence.
module.exports = function createInventoryService(repository, global = {}) {
    ...
    function loadDb() { return classifyLocations(repository.read()); }
    function saveDb(db) { repository.write(classifyLocations(db)); }
    ...
    const NebimAdapter = {
        ...
        seedProductsIfMissing() { return withDelay(true); },
        ...
        getWarehouseMatrixData() {
            ...
            return withDelay({ products: db.products, stores, token: repository.token(), fetchedAt: new Date().toISOString(), source: db.meta?.stockSource || 'central', sourceLabel: 'Merkezi veritabanı; Nebim bağlantısı yok.' });
        },

        applyWarehouseMatrix(warehouse, entries, token) {
            const db = loadDb();
            if (repository.token() !== token) throw new Error('Stok verileri değişti. Güncel veriyi tekrar çekin.');
        ...
        previewDistribution(config) {
            const db = loadDb();
            return withDelay({ ...global.DistributionEngine.build(db, config), token: repository.token() });
        },

        applyDistribution(config, token) {
            const db = loadDb();
            if (repository.token() !== token) throw new Error("Veriler değişti. Dağıtım önizlemesini yeniden oluşturun.");
            ...
            const known = new Set([...db.stores.map(s => s.name), ...db.products.map(p => p.store)]);
        ...
        transferStock(productId, targets, quantity, salesDateRange, remainderOptions = {}) {
            ...
            const known = [...db.stores.map(s => s.name), ...db.products.map(p => p.store)];
        ...
        getStats() {
            ...
            return withDelay({
                ...
                userCount: global.accountCount ?? db.users.length,
                transferCount: db.transfers.length
            });
        },

        // --- RESET (geliştirme/test amaçlı) ---
        resetToSeed() { throw new Error('Merkezi veriler demo verileriyle değiştirilemez.'); }
