(function (root) {
    ...
    function build(data, config) {
        ...
        const minimumQuantity = config.minimumQuantity ?? 1;
        if (config.mode !== 'chance' && !config.sendAllStock && (!Number.isSafeInteger(minimumQuantity) || minimumQuantity < 1)) return fail('Minimum transfer adedi pozitif tam sayı olmalıdır.');
        ...
        for (const p of relevant) {
            ...
            for (const to of ordered) {
                ...
                add(to, config.mode === 'winner' && config.minTwoStock ? 1 : config.sendAllStock ? remaining : Math.min(remaining, minimumQuantity), config.mode === 'winner' ? `Dönem satışı: ${salesTotal(to, key)} · Sıfır stok tamamlama` : 'Sıfır stok tamamlama');
            }
        ...
        if (config.mode === 'winner') result.notes.push('Eşit satışta hedef seçim sırası kullanılır; kalan kaynakta kalır.');
        if (config.mode !== 'chance') result.notes.push(config.sendAllStock ? 'Tüm stok ilk uygun hedefe gönderilir.' : `Her uygun hedefe ürün/renk/beden başına ${minimumQuantity} adet ayrılır.`);
