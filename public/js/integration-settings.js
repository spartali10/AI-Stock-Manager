(() => {
    ...
    async function request(path, options = {}) {
        // No Nebim network operations are enabled until a read-only connector exists.
        // In particular, never send the legacy POST connection-test request.
        if (/^\/integrations\/nebim(?:\/|$)/i.test(path)) throw new Error('Nebim yalnızca okuma modunda. Güvenli okuma bağlantısı henüz uygulanmadığı için Nebim isteği gönderilmedi.');
        if (path === '/integrations/api-keys' && options.method === 'POST') {
            const payload = JSON.parse(options.body || '{}');
            if (!['read', 'read_write'].includes(payload.scope) || (payload.scope === 'read_write' && load().api?.scope !== 'read_write')) throw new Error('Yalnızca okuma etkin. Yazma kapsamını önce ayarlardan kaydedin.');
        }
    ...
    function open(kind, button) {
        ...
        get('apiKeyScope').value = load().api?.scope === 'read_write' ? 'read_write' : 'read';
    ...
    get('integrationForm').addEventListener('submit', event => {
        event.preventDefault(); run(async () => {
            ...
            if (active === 'nebim' || active === 'api') { values.scope = 'read'; values.readOnly = true; }
            if (active === 'api' && get('apiKeyScope').value === 'read_write') {
                values.scope = 'read_write'; values.readOnly = false;
            }
    ...
    get('apiKeyCreate').addEventListener('click', () => run(async () => {
        ...
        const scope = load().api?.scope === 'read_write' ? 'read_write' : 'read';
        const data = await request('/integrations/api-keys', { method: 'POST', body: JSON.stringify({ name, scope }) });
